...
export const TagsInlineEditor = ({
  ...
  const tagIds$ = tagService.tagList.tagIdsByPageIds$(pageIds);
  ...
  const onSelectTag = useCallback(
    (tagId: string) => {
      tagService.tagList.tagByTagId$(tagId).value?.bulkTag(pageIds);
      onChange?.(tagIds$.value);
    },
    [onChange, pageIds, tagIds$, tagService.tagList]
  );

  const onDeselectTag = useCallback(
    (tagId: string) => {
      tagService.tagList.tagByTagId$(tagId).value?.bulkUntag(pageIds);
      onChange?.(tagIds$.value);
    },
    [onChange, pageIds, tagIds$, tagService.tagList]
  );
